...
# drifts if write == true (permits plotting of existing drifts in payProbDrift.csv)
...
if (write) {
    ...
    for (i in 2:totaltrials) {
        ...
        write.table(payoff, "payProbDrift.csv", row.names=F,
                    col.names=F, quote=F, sep=",")
    
} else {

       payoff <- read.table("payProbDrift.csv", header=F, sep=",")    

}

if (plotDrifts) {
        plot(payoff[,1], type="l", ylim=c(0.25, 0.75), ylab="probability", xlab="trial")
